...
namespace KJS {
   ...
   */
  template <typename FuncImp,typename ImpType>
  inline Value lookupOrCreateFunction(ExecState *exec, const Identifier &propertyName,
                                      const ImpType *thisObj, int token, int params, int attr)
  {
      ...
      ImpType *thatObj = const_cast<ImpType *>(thisObj);
      thatObj->put(exec, propertyName, val, attr);
      return val;
  }
